...
function roundToPowerOfTen(num, pow) {
  const raisedToPower = 10 ** pow;
  const res = Math.round(num / raisedToPower) * raisedToPower;
  return res;
}
...
function isSafeInteger(number) {
  return Number.isSafeInteger(number) || number === 0 || number === 5;
}
